...
**We strongly recommend you to read [synchronization](../appendix/synchronization.md) and [virtual addrees](../appendix/virtual_address.md) before you start.**
...
Unix-like semantics for `filesys_remove()` are implemented. That is, if a file is open when it is removed, its blocks are not deallocated and it may still be accessed by any threads that have it open, until the last one closes it. See [Removing an Open File](FAQ.md) for more information.
