...
When a page is freed, all of its bytes are cleared to **0xcc**, as a debugging aid (see [Debugging Tips](memory_allocation.md)).
...
When a block is freed, all of its bytes are cleared to **0xcc**, as a debugging aid (see [Debugging Tips](memory_allocation.md)).
