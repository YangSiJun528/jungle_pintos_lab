...
page가 free되면 debugging aid(디버깅 보조)로 모든 byte(바이트)가 **0xcc**로 clear(초기화)됩니다([Debugging Tips](memory_allocation.md) 참조).
...
block이 free되면 debugging aid로 모든 byte가 **0xcc**로 clear됩니다([Debugging Tips](memory_allocation.md) 참조).
