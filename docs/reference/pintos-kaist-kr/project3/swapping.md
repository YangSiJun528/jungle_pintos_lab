...
> disk에서 memory로 data content를 read(읽기)하여 swap disk에서 anonymous page를 swap in합니다. data location(위치)은 page가 swap out될 때 page struct에 저장되어 있어야 합니다. swap table(스왑 테이블)을 update(갱신)해야 한다는 점을 기억하세요([Managing the Swap Table](introduction.md) 참조).
