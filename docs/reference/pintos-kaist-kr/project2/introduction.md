...
**시작하기 전에 [synchronization](../appendix/synchronization.md)과 [virtual addrees](../appendix/virtual_address.md)를 읽는 것을 강하게 권장합니다.**
...
`filesys_remove()`에 대한 Unix-like semantics(유닉스식 의미)가 구현되어 있습니다. 즉, open(열린) 상태인 file이 remove(삭제)되면 그 block은 deallocate(할당 해제)되지 않으며, 그 file을 열고 있는 thread는 마지막 thread가 close(닫기)할 때까지 계속 접근할 수 있습니다. 자세한 내용은 [Removing an Open File](FAQ.md)을 참조하세요.
