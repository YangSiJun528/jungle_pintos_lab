...
**We strongly recommend you to read [synchronization](5_appendix__1_synchronization.md) and [virtual addrees](5_appendix__3_virtual_address.md) before you start.**
...
Unix-like semantics for `filesys_remove()` are implemented. That is, if a file is open when it is removed, its blocks are not deallocated and it may still be accessed by any threads that have it open, until the last one closes it. See [Removing an Open File](2_project2__7_FAQ.md) for more information.
