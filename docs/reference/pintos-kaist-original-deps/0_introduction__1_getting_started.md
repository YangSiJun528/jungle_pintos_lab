...
Following the build, the following are the interesting files in the `build` directory:
  ...
  - `kernel.o`: Object file for the entire kernel. This is the result of linking object files compiled from each individual kernel source file into a single object file. It contains debug information, so you can run GDB (see [GDB](5_appendix__5_debugging_tools.md#GDB)) or backtrace ([Backtraces](https://casys-kaist.github.io/pintos-kaist/appendix/debuuging_tools.md#Backtraces)) on it.
