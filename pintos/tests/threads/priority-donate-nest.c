...
/* 낮은 priority의 main thread L이 lock A를 획득한다. 그 다음 중간 priority의
   thread M이 lock B를 획득한 뒤 lock A를 획득하려다 block된다. 높은 priority의
   thread H는 lock B를 획득하려다 block된다. 따라서 thread H는 M에 priority를
   donation하고, M은 다시 thread L에 그 priority를 donation한다.
