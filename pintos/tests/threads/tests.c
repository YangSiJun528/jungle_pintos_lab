...
/* printf()처럼 FORMAT을 출력하되, 출력 앞에 테스트 이름을 붙이고 끝에 newline 문자를
   추가한다. */
void
...
/* printf()처럼 실패 메시지 FORMAT을 출력하되, 출력 앞에 테스트 이름과 FAIL:을 붙이고
   끝에 newline 문자를 추가한 다음 kernel panic을 발생시킨다. */
void
