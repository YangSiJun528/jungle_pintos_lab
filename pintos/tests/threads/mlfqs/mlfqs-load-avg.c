...
*/
/* 0부터 59까지 번호가 붙은 60개 thread를 시작한다. thread #i는 (10+i)초 동안
   sleep한 뒤 60초 동안 loop에서 spin하고, 총 120초가 지날 때까지 sleep한다. 시작
   10초 후부터 main thread는 2초마다 load average를 출력한다.

   원문 주석에는 예상 출력이 나열되어 있으며, 일부 오차는 허용된다.

   이 테스트는 대부분의 다른 테스트를 통과하지만 이 테스트만 실패한다면, timer
   interrupt 안에서 너무 많은 작업을 하고 있는지 고려하라고 설명한다. timer interrupt
   handler가 너무 오래 걸리면 테스트의 main thread가 자신의 작업인 메시지 출력 후
   다음 tick 전에 다시 sleep할 시간이 부족해진다. 그러면 tick이 도착할 때 main
   thread가 sleep 중이 아니라 ready 상태가 되어 load average가 인위적으로 올라갈 수
   있다. */
