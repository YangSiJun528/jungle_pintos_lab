...
/* blocked thread에 대해 recent_cpu와 priority가 갱신되는지 확인한다.

   main thread는 25초 동안 sleep하고, 5초 동안 spin한 뒤 lock을 release한다.
   "block" thread는 20초 동안 spin한 뒤 lock 획득을 시도하며, main thread가 lock을
   release할 때까지 10초 동안 block된다. "block" thread가 sleep하는 동안 recent_cpu가
   올바르게 decay된다면, main thread가 lock을 release할 때 block thread가 즉시
   schedule되어야 한다. */
