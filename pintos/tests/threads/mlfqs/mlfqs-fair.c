...
/* "nice" 구현의 정확성을 측정한다.
   ...
   "fair" 테스트는 nice가 모두 0인 thread 2개 또는 20개를 실행한다. 모든 thread는
   거의 같은 수의 tick을 받아야 한다. 각 테스트는 30초 동안 실행되므로 tick 합계도
   대략 30 * 100 == 3000 tick이 되어야 한다.

   mlfqs-nice-2 테스트는 thread 2개를 실행하며 하나는 nice 0, 다른 하나는 nice 5이다.
   이들은 30초 동안 각각 1,904 tick과 1,096 tick을 받아야 한다.

   mlfqs-nice-10 테스트는 nice 0부터 9까지의 thread 10개를 실행한다. 이들은 30초
   동안 각각 672, 588, 492, 408, 316, 232, 152, 92, 40, 8 tick을 받아야 한다.

   위 값들은 mlfqs.pm의 시뮬레이션으로 계산된다. */
