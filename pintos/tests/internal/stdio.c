...
*/
/* lib/stdio.c의 printf()용 test program.

   Pintos의 다른 곳에서 충분히 테스트되지 않는 printf() 기능을 테스트하려고 시도한다.
...
test (void) 
{
  ...
  /* Check that commas show up in the right places, for positive
     numbers. */
  /* 양수에서 comma가 올바른 위치에 나타나는지 확인한다. */
  checkf ("1", "%'d", 1);
  checkf ("12", "%'d", 12);
  checkf ("123", "%'d", 123);
  checkf ("1,234", "%'d", 1234);
  ...
  /* signed integer conversion을 확인한다. */
  checkf ("    0", "%5d", 0);
  checkf ("0    ", "%-5d", 0);
  checkf ("   +0", "%+5d", 0);
  checkf ("+0   ", "%+-5d", 0);
  ...
  /* unsigned integer conversion을 확인한다. */
  checkf ("    0", "%5u", 0);
  checkf ("    0", "%5o", 0);
  checkf ("    0", "%5x", 0);
  checkf ("    0", "%5X", 0);
  ...
  /* Cristian Cadar의 automatic test case generator에서 가져왔다. */
